...
func (s *WorldStore) seedWorld(ctx context.Context, world *WorldDocument, seedRoot, seedName string) error {
	...
	now := time.Now().UTC()
	resources := []any{WorldResource{WorldID: world.ID, Kind: "collection", Key: "source", Revision: 1, Content: collectionJSON, Size: len(collectionJSON), UpdatedAt: now}, WorldResource{WorldID: world.ID, Kind: "palette", Key: "colors", Revision: 1, Content: paletteJSON, Size: len(paletteJSON), UpdatedAt: now}, WorldResource{WorldID: world.ID, Kind: "manifest", Key: "world", Revision: 1, Content: manifestJSON, Size: len(manifestJSON), UpdatedAt: now}}
